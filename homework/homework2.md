...
![alt text](https://drive.google.com/file/d/11WO0-Iz60Ej_yod26tQJ-XJrkwlRFuL5/view?usp=drive_link)

### Question 2
Carefully explain the differences between the KNN classifier and the KNN regression methods. <br> 
KNN is  non parametric method. KNN classifier used for classification problems or cluster data whereas KNN regression methods are used for predicting numerical values. KNN classifier sets the class of the unknown sample by taking the class of the k most similar examples in the training dataset. KNN can identify complex patterns and sometime it is challenging to interpret its output. Unlike other methods, KNN regression predicts a value for a new data point by finding its K closest neighbors in the training data. Then, it combines (averages or uses another method) the values of those neighbors to come up with an estimated value for the new point.<br> 

### Question 3
We cover the SNE, symmetric SNE and t-SNE in the class.
* (a) What does the SNE algorithm try to minimize? <br>

tsne does analysis in two steps. In first step it tries to make a probability distribution over high dimensional datapoints where same classes are assigned to higher probability and different objects are classified as lower probability. In second step, t-SNE does a similary probability distribution over the points in the low-dimensional map. It minimizes the Kullback-Leibler divergence between the two disbutions using euclidean distance. <br>
...
![alt text](https://drive.google.com/file/d/19_z0xhTuXmJ5VX_mAoflxUbqcev34Ok0/view?usp=drive_link)
