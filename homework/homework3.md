...
The probability of picking the jth sample is 1/n and the total chance of getting a sample is 1. So the chance of the first bootstrap observation that is not $j$th observation is 1 -1/n. 
...
Since bootstrap include repetition re sampling so the probability of the second bootstrap observation is not $j$th observation is 1 -1/n. 

3. Argue that the probability that the $j$th observation is not in the bootstrap sample is $(1-1/n)^n$.
It's because each observation in the bootstrap sample is drawn independently with replacement from the original sample. so the probability will be (1-1/n) * (1-1/n) ...*(1-1/n) = 1-1/n)^n

4. When $n = 5$, what is the probability that the $j$th observation is in the bootstrap sample? 
P($j$th observation is in the bootstrap samplee)=1−(1−1/5​^)5=0.672
5. When $n = 100$, what is the probability that the $j$th observation is in the bootstrap sample?
P($j$th observation is in the bootstrap sample)=1−(1−1/100​^1005=0.342
6. When $n = 10000$, what is the probability that the $j$th observation is in the bootstrap sample?
P($j$th observation is in the bootstrap sample)=1−(1−1/10000)10000=0.632.
...
K-fold validation has n number of observations and k number of non-overlappping groups. The length of the groups is n/k. the n-n/k acts as training set and the remaining work as validation set. The test error is estimated averaging the MSE estimates of the k groups. 
...
LOOCV will have limited data usage with high computational cost and high variance. Where k-fold validation splits the data into different folds and keep one for n/k validation and rest for the training. 
...
df <- data.frame(p, gini.index, class.error, cross.entropy)

# Plot using ggplot
ggplot(df, aes(x = p)) +
geom_line(aes(y = gini.index, color = "Gini Index")) +
geom_line(aes(y = class.error, color = "Class Error")) +
geom_line(aes(y = cross.entropy, color = "Cross Entropy")) +
ylab("Value") +
scale_color_manual(name = "Metrics", values = c("Gini Index" = "green", "Class Error" = "blue", "Cross Entropy" = "orange")) +
theme_minimal()

![alt text](https://git.uark.edu/rbmahbub/math-499-v-599-v-mfml-riasad-mahbub/-/blob/main/homework/giniinddex.png)
...
The majority approach is mode and the averagae approach is median. In majority approach it is green, 6 obverstations are green but however by average approach it is red if we consider 0.5 as the threshold. 
...
I am working with my own data. I want to implement random forest algorithm on 255 data observation points. I have a training set consisting of 60% of the data, testing set consisiting of 20% of the data, and validation set which consist of 20% more of the data. I am using satellite image data which are converted to numbers. The numbers are different reflectance values of the study area and used to train the model. I am trying to predict the rice fields are planted and when are they harvested based on this information. Next I want to implement k-fold cross validation on the training and validation set. 
